...
func (client *Client) GetJobID(jobUUID string) (string, error) {
	resp, err := client.Do(`
		query($uuid:ID!) {
			job(uuid: $uuid) {
				... on JobTypeCommand {
	...
	return parsedResp.Data.Job.ID, nil
}

func (client *Client) ChangeJobQueryRule(jID string, query string) error {
	_, err := client.Do(`
        mutation ChangeJobQueryRule($jobID:ID!, $agentQueryRules: [String]) {
            jobTypeCommandUpdate(input:{id: $jobID, agentQueryRules: $agentQueryRules}) {
                jobTypeCommand {
                    agentQueryRules
                }
            }
